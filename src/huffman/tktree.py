...
class tktree:
    ...
    def __init__(self, title="", font=6, width=30, height=1.2, spacex=0.5, spacey=0.5, sick=1, double=False):
        self.font = font
        self.width = width
        self.win = tk.Tk()
    ...
    def canv(self, t):
        ...
        if not self.double:
            ...
            genTreeH(self.cv, False, self.spacex, w2, h2, self.sick, self.height, w2 * self.spacex,
            ...
            self.cv.create_line(w / 2 - w2 * 3 / 2 - self.spacex, self.spacey * h2 + h / 2,
                                ...
                                fill="lightgrey", width=self.sick)
            rect(self.cv, False, w2, h2, self.sick, self.height, w / 2 - w2 / 2,
                 self.spacey * h2 + h / 2, h, t)
            genTreeH(self.cv, False, self.spacex, w2, h2, self.sick, self.height,
                     w / 2 + w2 * (self.spacex + 1 / 2), self.spacey * h2 + h / 2, h, t.children[0])
            genTreeH(self.cv, True, self.spacex, w2, h2, self.sick, self.height,
                     w / 2 - w2 * (self.spacex + 1 / 2), self.spacey * h2 + h / 2, h, t.children[1])


def genTreeH(cv, rev, spcx, w2, h2, sk, fh, x0, y0, dy, t):
    rect(cv, rev, w2, h2, sk, fh, x0, y0, dy, t)
    st = t.getSize()
    dy0 = y0 - dy / 2
    if not isinstance(t, leaf.leaf):
        for child in t.children:
            ...
            if rev:
                genTreeH(cv, rev, spcx, w2, h2, sk, fh, x0 - w2 *
                         (1 + spcx), dy0 + hc / 2, hc, child)
                cv.create_line(
                    x0 - w2, y0, x0 - w2 * (1 + spcx), int(dy0 + hc / 2), fill="lightgrey", width=sk)
            else:
                genTreeH(cv, rev, spcx, w2, h2, sk, fh, x0 + w2 *
                         (1 + spcx), dy0 + hc / 2, hc, child)
                cv.create_line(
...
def rect(cv, rev, w2, h2, sk, fh, x0, y0, dy, t):
    color = "white"
    ...
    if rev:
        x0 -= w2
    cv.create_rectangle(
        x0, int(y0 - h2 / 2), x0 + w2, int(y0 + h2 / 2), outline="lightgrey", fill=color, width=sk)
    cv.create_text(
