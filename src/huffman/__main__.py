...
def compress(src, dest):
    ...
    c.load(src)
    print("Source file loaded in {.2f} ms".format(c.stats['loadingTime']*1000))
    print("Encoding source file...")
    c.encode()
    print("Source file encoded in {.2f} ms".format(c.stats['processTime']*1000))
    print("Writing destination file...")
    c.write(dest, enc=True)
    print("Destination file written in {.2f} ms".format(c.stats['processTime']*1000))
...
def decompress(src, dest):
    ...
    c.encode()
    print("Source file decoded in {.2f} ms".format(c.stats['processTime']*1000))
    print("Writing destination file...")
    c.write(dest, enc=True)
    print("Destination file written in {.2f} ms".format(c.stats['processTime']*1000))
